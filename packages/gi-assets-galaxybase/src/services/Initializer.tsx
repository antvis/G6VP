...
export const GI_SERVICE_INTIAL_GRAPH = {
  name: $i18n.get({ id: 'galaxybase.src.services.Initializer.InitializeAQuery', dm: '初始化查询' }),
  service: async () => {
...
export const GI_SERVICE_SCHEMA = {
  name: $i18n.get({ id: 'galaxybase.src.services.Initializer.QueryGraphModel', dm: '查询图模型' }),
  service: async params => {
    ...
    if (!GALAXYBASE_USER_TOKEN) {
      ...
      message.error(
        $i18n.get({
          id: 'galaxybase.src.services.Initializer.GalaxybaseDataSourceConnectionFailed',
          dm: 'Galaxybase 数据源连接失败: 没有获取到连接 Galaxybase 数据库的 Token 信息，请先连接 Galaxybase 数据库再进行尝试！',
        }),
      );
      return;
    }
