...
export const shouldUseDagre: RuleModule = {
  ...
  validator: (graphFeat: IGraphFeat):number => {
    let score = 0
...
export const shouldUseForce: RuleModule = {
  ...
  validator: (graphFeat: IGraphFeat):number => {
    let score = 0
    const { nodeCount, edgeCount, maxDegree, avgDegree, strongConnectedComponentCount } = graphFeat
    if(nodeCount > 200) {
      score += 4
    }
...
export const layoutTypePredRule: RuleModule = {
  ...
  optimizer: (dataProps: IGraphProps):LayoutTypes => {
    ...
    const allLayoutTypeRules = {
      ...
      grid: shouldUseGrid, 
      'graphin-force': shouldUseForce // TODO force似乎目前不可用，先使用 graphin-force
    }
    const candidates = []
