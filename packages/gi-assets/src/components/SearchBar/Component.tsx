...
const SearchBar: React.FC<ISearchBarProps> = ({ serviceId, schemaServiceId, style }) => {
  ...
  const handleQueryByGQL = async (filterStr: string) => {
    setBtnLoading(true);
    const gql = `MATCH (n${currentSchemaType ? `:${currentSchemaType}` : ''} WHERE ${currentAttr} ${filterStr}) RETURN n`
    // 获取查询的 ISO GQL
  ...
  return (
    <div className='searchContainer' style={style}>
      <Space>
        ...
        >
          ...
          {schemaTypeData?.map(std => (
            <Option value={std} key={std}>{std}</Option>
          ))}
        </Select>
        ...
        >
          {schemaAttr.map(sa => (
            <Option value={sa} key={sa}>{sa}</Option>
          ))}
        </Select>
        <Search
          style={{ width: 145 }}
      ...
      <pre className='codes'>MATCH (n{currentSchemaType ? `:${currentSchemaType}` : ''} WHERE {currentAttr} {gqlFilter}) RETURN n</pre>
    </div>
  );
};
