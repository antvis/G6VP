...
const GraphScopeMode: React.FC<GraphModelProps> = ({ onClose }) => {
  ...
  const graphScopeFilesMapping = JSON.parse(localStorage.getItem(
    "graphScopeFilesMapping"
  ) as string);
  ...
  const initGraphScopeInstance = async () => {
    ...
    const gsResult = await createGraphScopeInstance.service();
  ...
  const handleUploadFiles = async (isCover = false) => {
    ...
    const nodeFilePromise = nodeFileLists.map(d => {
      // 上传点文件
      const nodeFileResult = uploadLocalFileToGraphScope.service({
        fileList: d,
    ...
    const edgeFileLists = edgeConfigList
      .filter(
        d =>
          d.edgeType && d.edgeFileList && d.sourceNodeType && d.targetNodeType
      )
      .filter(d => {
        // 过滤到不完整的配置后，还要再过滤掉已经上传过的文件
        if (!isCover && graphScopeFilesMapping) {
    ...
    const edgeFilePromise = edgeFileLists.map(d => {
      // 上传点文件
      const edgeFileResult = uploadLocalFileToGraphScope.service({
        fileList: d,
        instanceId: currentInstanceId
      });
    ...
    const filesUploadResult = await Promise.all([
      ...nodeFilePromise,
      ...edgeFilePromise
    ]);
  ...
  const updateSchemaData = async modeType => {
    ...
    const result = await queryGraphSchema.service();
  ...
  const handleSubmitForm = async () => {
    ...
    // 使用示例数据
    if (dataType === "demo") {
      const loadResult = await loadDefaultGraphToGraphScope.service({
    ...
    if (dataType === "chinavis") {
      // loadChinaVisGraphToGraphScope
      const loadResult = await loadChinaVisGraphToGraphScope.service({
    ...
    const loadResult = await loadGraphToGraphScope.service({
      nodeConfigList,
      edgeConfigList,
      ...
      <Form name="gsform" form={form} initialValues={formInitValue}>
        ...
        {dataType === "chinavis" && (
          <div style={{ marginTop: 16 }}>
            ...
            <p>边文件：Link.csv</p>
            <p>
              测试数据共包括 237 万个与黑灰产相关的网络资产（节点）和 328
              万条资产关联关系
            </p>
            <p>点类型：</p>
            <Table
