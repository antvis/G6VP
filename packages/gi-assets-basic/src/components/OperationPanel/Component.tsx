...
const OperationPanel: React.FC<Props> = (props) => {
  const { groups,size = 60,position = 'Top',left,right,top,bottom,className: propClassName,style: propStyle } = props;
  ...
  const className = `gi-operation-panel ${propClassName || ''}`;
  ...
  const [containerAttr] = React.useState(() => `with-operation-panel-${index++}`);
    ...
    <style type='text/css' ref={cssRef}/>
    {
      groups?.map((group,index) => {
        ...
        const driverElements = drivers?.map((driver,index) => {
          ...
          return <i key={`driver_${index}`} className='gi-operation-panel-group-driver' style={driverStyle}/>
        });
      
        return <div className={'gi-operation-panel-group'} key={index} style={style}>
          {
